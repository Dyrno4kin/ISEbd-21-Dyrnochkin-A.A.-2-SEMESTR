...
namespace FishShopServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
        ...
        public ReportServiceDB(FishDbContextWPF context)
        {
        ...
        public void SaveCanFoodPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var CanFoods = context.CanFoods.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, CanFoods.Count, 2, ref
               missing, ref missing);
                ...
                for (int i = 0; i < CanFoods.Count; ++i)
                {
                    table.Cell(i + 1, 1).Range.Text = CanFoods[i].CanFoodName;
                    table.Cell(i + 1, 2).Range.Text = CanFoods[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
            ...
            .ToList()
           .GroupJoin(
            context.StockIngredients
            .Include(r => r.Ingredient)
           .ToList(),
            stock => stock,
           stockIngredient => stockIngredient.Stock,
           (stock, stockIngrList) =>
            new StocksLoadViewModel
            {
                StockName = stock.StockName,
                TotalCount = stockIngrList.Sum(r => r.Count),
                Ingredients = stockIngrList.Select(r => new Tuple<string,
    int>(r.Ingredient.IngredientName, r.Count))
        ...
        public void SaveCustomerOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            table.AddCell(new PdfPCell(new Phrase("Консерва", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CanFoodName, fontForCells));
                table.AddCell(cell);
